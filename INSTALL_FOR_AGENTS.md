...
2. Copy `<plugin>/connectors/config.example.json` to the user's project (suggested: `.claude/connectors/quill.json`) and set `"source_name": "quill"`. The defaults work as-is. Ask the user which optional features they want, and complete each one's precondition yourself:
   - **`llm_enabled: true`** (LLM-formatted transcripts, classification, action items) — needs the `anthropic` package (`pip install anthropic`) and an `OPENROUTER_API_KEY` (https://openrouter.ai/keys; the default `llm_model` `openrouter/auto` uses OpenRouter's automatic routing). Ask the user for the key, write it to a file as a single `OPENROUTER_API_KEY=...` line (never inline in the config, never in chat), point `env_file` at that file's absolute path, and make sure the file is gitignored. Without this the sync still works — transcripts are written verbatim without summaries.
