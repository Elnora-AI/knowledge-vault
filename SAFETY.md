...
- LLM formatting, if enabled, uses your own LLM API key (Anthropic, OpenAI, Google, OpenRouter, Groq, DeepSeek, xAI, Mistral, or any OpenAI-compatible endpoint you configure) via environment variables — never committed, never logged. A connector `env_file` keeps keys in a file you control.
